...
| 层级 | 技术 | 说明 |
|------|------|------|
| 前端 | React 19 + Vite + CSS | 深色毛玻璃 UI，含地址输入、可收起侧边栏授权管理（4 类别独立授权/撤销/ZK哈希）、聊天、中英切换 |
...
| 模块 | 完成项 | 状态 |
|------|--------|------|
| 前端 | 地址输入 + 侧边栏授权管理（可收起） + 聊天 + 数据类别 + 中英切换 | ✅ Done |
...
| 部署费用 | 5.19 credits (v2) / ~5.31 credits (v3 编译通过，Testnet 费率偏差暂阻部署) |
